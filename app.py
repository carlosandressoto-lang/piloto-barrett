...
if df is not None:
    ...
    with cnb1:
        ...
        color_dyn = "white" if theme_is_dark else "black"
        ...
        for x0, x1, y0, y1, color, label in cuadrantes_specs:
            ...
            fig_nb.add_annotation(x=(x0+x1)/2, y=y1-2, text=f"<b>{label}</b>", showarrow=False, font=dict(size=8, color=color_dyn))

        # Lógica de Posicionamiento dinámico de la leyenda
        val_p = d.IND_POT
        ...
        fig_nb.add_trace(go.Scatter(
            ...
            textfont=dict(size=9, color=color_dyn)
        ))
