...
if df is not None:
    ...
    st.subheader("📊 Gráficos Barrett")
    c1, c2, c3 = st.columns(3)
    with c1: st.plotly_chart(generar_fig_barras(v_auto, "Auto", "#3498db"))
    with c2: st.plotly_chart(generar_fig_barras(v_ind, "Individual", "#2ecc71"))
    with c3: st.plotly_chart(generar_fig_barras(v_org, "Organizacional", "#e74c3c"))
    ...
    with cnb1:
        ...
        vp = d.IND_POT
        pos = "bottom center" if (54 <= vp <= 60 or 74 <= vp <= 80 or vp >= 90) else "top center"
        ...
        fig_nb.add_trace(go.Scatter(
            ...
            textposition=pos,
            textfont=dict(size=10, color="black"), # NEGRO ESTÁTICO
            hoverinfo="all"
        ))
        fig_nb.update_layout(xaxis=dict(tickvals=[1,2,3], range=[0.5, 3.5]), yaxis=dict(tickvals=[0, 33, 66, 100], range=[-5, 105]), template="plotly_dark", height=500)
    ...
    if st.button("🚀 GENERAR INFORME"):
        prompt = f"Genera reporte Barrett para {lider_sel}. DATOS: {d.to_json()}"
        try:
    ...
    if lider_sel in st.session_state.informe_cache:
        if st.button("📄 GENERAR PDF"):
            try:
                ...
                st.download_button("Descargar", pdf.output(dest='S'), f"Reporte_{lider_sel}.pdf", "application/pdf")
