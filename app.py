...
if df is not None:
    ...
    with c3: st.plotly_chart(generar_fig_barras(v_org, "Organizacional", "#e74c3c"), key="b3_v")
    ...
    with cnb1:
        ...
        # Lógica de posición inteligente
        val_p = d.IND_POT
        ...
        fig_nb.update_layout(xaxis=dict(tickvals=[1,2,3], range=[0.5, 3.5]), yaxis=dict(tickvals=[0, 33, 66, 100], range=[-5, 105]), template="plotly_dark" if theme_is_dark else "plotly", height=500)
    ...
    if st.button("🚀 GENERAR INFORME"):
        prompt_maestro = f"""Actúa como consultor Barrett. Genera un reporte para {lider_sel}. DATOS: {d.to_json()}... (Prompt original)"""
        try:
