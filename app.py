...
def generar_fig_reloj(vals, incluir_leyenda=False):
    ...
    fig.update_layout(
        height=400, margin=dict(l=100 if incluir_leyenda else 10, r=20, t=10, b=10), 
        yaxis=dict(visible=incluir_leyenda, tickfont=dict(size=10)), 
...
if df is not None:
    ...
    with c1: st.plotly_chart(generar_fig_barras(v_auto, "Autoevaluación", "#3498db"))
    with c2: st.plotly_chart(generar_fig_barras(v_ind, "Ponderado Individual", "#2ecc71"))
    with c3: st.plotly_chart(generar_fig_barras(v_org, "Ponderado Organizacional", "#e74c3c"))
    ...
    with cnb1:
        ...
        for x0, x1, y0, y1, color, label in cuadrantes:
            fig_nb.add_shape(type="rect", x0=x0, y0=y0, x1=x1, y1=y1, fillcolor=color, opacity=0.4, line=dict(color="white", width=1.5))
            fig_nb.add_annotation(x=(x0+x1)/2, y=y1-2.5, text=f"<b>{label}</b>", showarrow=False, font=dict(size=10, color="black"))

        fig_nb.add_trace(go.Scatter(
            x=[d.DES], y=[escalar_visual_potencial(d.IND_POT)], mode='markers',
            marker=dict(size=16, color='white', symbol='diamond', line=dict(width=3, color='black')), 
            hoverinfo="text",
            text=f"Potencial: {round(d.IND_POT,2)}%<br>Desempeño: {d.DES}"
        ))
        fig_nb.update_layout(xaxis=dict(title="Desempeño (1-3)", tickvals=[1,2,3], range=[0.5, 3.5]), yaxis=dict(title="Potencial", tickvals=[0, 33.3, 66.6, 100], range=[-5, 105]), plot_bgcolor='white', paper_bgcolor='white', height=500)
        st.plotly_chart(fig_nb, use_container_width=True)

    # --- INFORME Y PDF HD ---
    if st.button("🚀 GENERAR INFORME"):
        prompt = f"Actúa como consultor Barrett. Genera reporte detallado para {lider_sel}. DATOS: {d.to_json()}"
