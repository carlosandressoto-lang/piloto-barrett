...
<style>
    ...
    /* Contenedor de leyenda alineado con el gráfico */
    .leyenda-v3 {
        display: flex;
...
if df is not None:
    ...
    # --- 6. RELOJES DE ARENA (RESPONSIVIDAD CORREGIDA) ---
    st.divider()
    ...
    with col_l:
        ...
        st.markdown('<div class="leyenda-v3">' + 
                    ''.join([f'<div class="item-ley">{n}</div>' for n in ["L7 - Visionario", "L6 - Mentor", "L5 - Auténtico", "L4 - Facilitador", "L3 - Desempeño", "L2 - Relaciones", "L1 - Crisis"]]) + 
                    '</div>', unsafe_allow_html=True)

    with col_r1:
        st.markdown('<div class="titulo-col">Autovaloración</div>', unsafe_allow_html=True)
        st.plotly_chart(dibujar_reloj_limpio(v_auto), use_container_width=True)
    with col_r2:
        st.markdown('<div class="titulo-col">Individual</div>', unsafe_allow_html=True)
        st.plotly_chart(dibujar_reloj_limpio(v_ind), use_container_width=True)
    with col_r3:
        st.markdown('<div class="titulo-col">Organizacional</div>', unsafe_allow_html=True)
        st.plotly_chart(dibujar_reloj_limpio(v_org), use_container_width=True)
