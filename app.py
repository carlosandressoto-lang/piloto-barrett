...
<style>
    ...
    h3 { text-align: center; margin-bottom: 5px !important; }
</style>
...
if df is not None:
    ...
    # --- 6. RELOJES DE ARENA (SISTEMA INTEGRAL UNIFICADO) ---
    st.divider()
    st.subheader("⏳ Evolución del Liderazgo (Escala de Madurez)")
    
    def dibujar_reloj_unificado(vals, titulo, mostrar_leyenda=False):
        levels_names = ["L7 - Visionario", "L6 - Mentor", "L5 - Auténtico", "L4 - Facilitador", "L3 - Desempeño", "L2 - Relaciones", "L1 - Crisis"]
        anchos = [6, 5, 4, 3.5, 4, 5, 6] 
        colors = ["rgba(111, 66, 193, 0.4)"]*3 + ["rgba(40, 167, 69, 0.4)"] + ["rgba(253, 126, 20, 0.4)"]*3
        
        # Invertimos valores para que L7 esté arriba en el Funnel
        ...
        fig.update_layout(
            ...
            height=450,
            # Si es el primer gráfico, dejamos espacio para la leyenda a la izquierda
            margin=dict(l=150 if mostrar_leyenda else 10, r=10, t=50, b=10),
            yaxis=dict(
                visible=mostrar_leyenda, # Solo mostramos los nombres en el primero
                autorange="reversed",
                tickfont=dict(color="#94a3b8", size=11, weight="bold")
            ),
    ...
    # El primero lleva la leyenda, los otros no
    with r1: st.plotly_chart(dibujar_reloj_unificado([d.AUTO_L1, d.AUTO_L2, d.AUTO_L3, d.AUTO_L4, d.AUTO_L5, d.AUTO_L6, d.AUTO_L7], "Auto", mostrar_leyenda=True), use_container_width=True)
    with r2: st.plotly_chart(dibujar_reloj_unificado([d.INDIV_L1, d.INDIV_L2, d.INDIV_L3, d.INDIV_L4, d.INDIV_L5, d.INDIV_L6, d.INDIV_L7], "Individual", mostrar_leyenda=False), use_container_width=True)
    with r3: st.plotly_chart(dibujar_reloj_unificado([d.ORG_L1, d.ORG_L2, d.ORG_L3, d.ORG_L4, d.ORG_L5, d.ORG_L6, d.ORG_L7], "Cultura", mostrar_leyenda=False), use_container_width=True)
