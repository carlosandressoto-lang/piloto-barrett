...
def generar_fig_reloj(vals, incluir_leyenda=False, forzar_pdf=False):
    ...
    # LA SOLUCIÓN QUE YA HABÍAMOS ARREGLADO (Reserva de margen l=100)
...
if df is not None:
    ...
    with cr1: st.plotly_chart(generar_fig_reloj(v_auto, incluir_leyenda=False), use_container_width=True)
    with cr2: st.plotly_chart(generar_fig_reloj(v_ind, incluir_leyenda=False), use_container_width=True)
    with cr3: st.plotly_chart(generar_fig_reloj(v_org, incluir_leyenda=False), use_container_width=True)
    ...
    if st.button("🚀 GENERAR INFORME"):
        ...
        REGLAS DE ORO: INICIA DIRECTAMENTE. SIN SALUDOS. FEEDBACK APRECIATIVO. RÚBRICA: Bajo (<65), Medio (65-75), Alto (75-85), Superior (>85).
        ESTRUCTURA OBLIGATORIA: 1. DESCRIPCIÓN POR NIVELES. 2. ANÁLISIS AUTOVALORACIÓN. 3. MATRIZ DE MADUREZ. 4. PERFIL DE LIDERAZGO. 5. POSICIONAMIENTO ESTRATÉGICO TALENTO ({cuadrante})."""
        try:
    ...
    # BLOQUE PDF: RESTAURACIÓN DE LA SOLUCIÓN DE SIMETRÍA
        ...
        if st.button("📄 DESCARGAR REPORTE PDF"):
            try:
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    def save_chart_pdf(fig, name, title=""):
                        fig.update_layout(template="plotly", paper_bgcolor='white', plot_bgcolor='white', font=dict(color='black'), title=dict(text=title, x=0.5, font=dict(size=14), y=0.95), margin=dict(t=60, b=20, l=60, r=20), autosize=False, width=500, height=350)
                    ...
                    img1 = save_chart_pdf(generar_fig_barras(v_auto, "", "#3498db"), "b1.png", "Autovaloracion")
                    img2 = save_chart_pdf(generar_fig_barras(v_ind, "", "#2ecc71"), "b2.png", "Individual")
                    img3 = save_chart_pdf(generar_fig_barras(v_org, "", "#e74c3c"), "b3.png", "Organizacional")
                    y_b = pdf.get_y(); pdf.image(img1, x=10, y=y_b, w=60); pdf.image(img2, x=75, y=y_b, w=60); pdf.image(img3, x=140, y=y_b, w=60)
                    
                    # 2. RADAR Y EQUILIBRIO
                    pdf.ln(45); pdf.cell(0, 10, '2. Alineacion e Indice de Equilibrio', ln=True)
                    ...
                    p_rad = os.path.join(tmp_dir, "radar.png"); fig_radar.write_image(p_rad, scale=2)
                    p_dim = os.path.join(tmp_dir, "dim.png"); fig_dim.write_image(p_dim, scale=2)
                    pdf.image(p_rad, x=10, w=90); pdf.image(p_dim, x=110, y=pdf.get_y()-65, w=90)

                    # 3. RELOJES (ESTRATEGIA VALIDADA DE SIMETRÍA)
                    pdf.add_page(); pdf.set_font('Helvetica', 'B', 11); pdf.cell(0, 10, '3. Niveles de Madurez Barrett (Relojes)', ln=True)
                    # El secreto: r1 lleva leyenda, r2 y r3 llevan forzar_pdf=True para reservar el mismo espacio
                    img_r1 = save_chart_pdf(generar_fig_reloj(v_auto, incluir_leyenda=True), "r1.png", "Autovaloracion")
                    img_r2 = save_chart_pdf(generar_fig_reloj(v_ind, incluir_leyenda=False, forzar_pdf=True), "r2.png", "Individual")
                    img_r3 = save_chart_pdf(generar_fig_reloj(v_org, incluir_leyenda=False, forzar_pdf=True), "r3.png", "Organizacional")
                    y_r = pdf.get_y(); pdf.image(img_r1, x=15, y=y_r, w=60); pdf.image(img_r2, x=75, y=y_r, w=60); pdf.image(img_r3, x=135, y=y_r, w=60)

                    # 4. NINEBOX Y TEXTO
                    pdf.add_page(); pdf.cell(0, 10, '4. Mapa NineBox y Analisis IA', ln=True)
                    fig_nb.update_layout(template="plotly", paper_bgcolor='white', plot_bgcolor='white', font=dict(color='black'))
                    p_nb = os.path.join(tmp_dir, "nb.png"); fig_nb.write_image(p_nb, scale=4); pdf.image(p_nb, x=25, w=160)
