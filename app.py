...
if df is not None:
    ...
    # --- 6. RELOJES DE ARENA (CON SEMÁFORO DE TEXTO Y CONTRASTE MEJORADO) ---
    st.divider()
    ...
    def obtener_etiqueta_color(v):
        ...
        return "Superior", "#3498db"            # Azul (Igual a Autovaloración)
    ...
    # Usamos columnas para centrar el radar
    ...
    # --- 8. INFORME IA A ANCHO COMPLETO ---
    st.divider()
    if st.button("✨ GENERAR INFORME EJECUTIVO"):
        prompt = f"Analiza los resultados 360 de {lider_sel} bajo el modelo Barrett: {d.to_json()}. Usa rúbrica 0-100. Compara Autopercepción, Individual y Organizacional de forma ejecutiva y profunda."
        try:
            with st.spinner('Analizando datos de liderazgo...'):
                response = model.generate_content(prompt)
                st.markdown(f"### 📋 Informe Ejecutivo: {lider_sel}")
                st.markdown("---")
                # El informe ahora ocupa todo el ancho de la pantalla
