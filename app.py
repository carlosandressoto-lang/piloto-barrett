...
if df is not None:
    ...
    with col_dim:
        ...
        fig_dim = go.Figure(go.Bar(x=vals_dim, y=['Liderazgo (L5-L7)', 'Transición (L4)', 'Gerencia (L1-L3)'], orientation='h', marker_color=[obtener_color_desarrollo(v) for v in vals_dim], text=[f"{round(v,1)}%" for v in vals_dim], textposition='inside'))
        fig_dim.update_layout(xaxis_range=[0, 105], height=400, template="plotly_dark", yaxis=dict(autorange="reversed"))
        st.plotly_chart(fig_dim, use_container_width=True)

    # --- BLOQUE 4: NINEBOX HD CON LEYENDA ---
    st.divider()
    ...
    with cnb1:
        ...
        fig_nb.update_layout(
            xaxis=dict(title="Desempeño (1-3)", tickvals=[1,2,3], range=[0.4, 3.6]), 
            yaxis=dict(title="Potencial (%)", tickvals=[0, 33.33, 66.66, 100], ticktext=["0", "60", "80", "100"], range=[-5, 105]),
    ...
    with cnb2:
        ...
        <div class="metric-box">
            ...
            <br>
            <p style="font-size:0.8rem; color: #94a3b8;">Análisis estratégico basado en Talent Confa 2026.</p>
        </div>
        """, unsafe_allow_html=True)

    # --- BLOQUE 5: INFORME Y PDF HD ---
    st.divider()
    if st.button("🚀 GENERAR INFORME"):
        prompt = f"""Actúa como consultor senior Barrett. Genera un reporte de 5 puntos para {lider_sel}. DATOS: {d.to_json()}... (Prompt original de 5 puntos solicitado anteriormente)"""
        try:
    ...
    if lider_sel in st.session_state.informe_cache:
        if st.button("📄 DESCARGAR PDF"):
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                # El fondo claro solo para la exportación PDF
                ...
                fig_nb.write_image(nb_path, engine="kaleido", scale=4) # HD Real
